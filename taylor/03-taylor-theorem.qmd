...
::: {.callout-note title="Example bad linear approximation" collapse="false"}
...
![Here $T_2(x)=f(x)$, while the straight line $T_1(x)$ is a poor approximation.](../figures/Taylor_bad.png){#fig-taylor-bad width=50%}
...
::: {.callout-tip collapse="true" title="Cauchy's Mean Value Theorem"}
...
::: {.callout-tip collapse="true" title="Technical Lemma"}
...
$\square$
:::
::: {.callout-tip collapse="true" title="Proof of Taylor's Theorem"}
...
$$
Because polynomial $T_n$ is of order $n$, we have $T_n^{(n+1)}(x)\equiv 0$. So this means that $R_n^{(n+1)}(\xi_x)=f^{(n+1)}(\xi_x)$, which gives the desired formula. \hskip 1cm $\square$
:::
