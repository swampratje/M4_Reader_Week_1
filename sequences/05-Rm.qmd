...
::: {.callout-tip title="Properties of norms, general definition" collapse="true"}
...
The notion of converging sequences in $\mR$ is easily extended to sequences in $\mR^m$. Now, a converging sequence in $\mR^m$ *closes in* on some vector $L\in\mR^m$. This means that by picking high enough indices, the distance between the corresponding row elements and $L$ become arbitrary small. We will measure the distance between vectors by the *Euclidean norm* $\Vert\cdot \Vert$. Formally,
...
Then we see that the sequence of the first coordinates $\{a_{n,1}\}_{n\in \mathbb{N}}=\{1+\tfrac{1}{n}\}_{n\in \mathbb{N}}$ converges with limit $1$, whereas the sequence of second coordinates \({\{a_{n,2}\}}_{{n\in \mN}}\) does not converge as $a_{n,2}\rightarrow \infty$ for $n\rightarrow\infty$. The sequence $\{a_n\}$ does not get close to a particular element of $\mathbb{R}^2$ for $n\rightarrow\infty$ , so we would intuitively say that it does not converge. In fact, for any fixed $L=(L_1,L_2)\in \mathbb{R}^2$,
$$
\Vert a_n-L\Vert = \sqrt{(1-L_1+\tfrac{1}{n})^2+(n-L_2)^2}\rightarrow \infty.
$$
:::

::: {.callout-tip title="A nice property of norms on $\mathbb{R}^m$" collapse="true"}
...
::: {#thm-coord}
## Coordinate-wise convergence criterion
